...
test.describe("Take screenshots for Visual Regression Testing - Modern page", () => {
  ...
  test(
    "Modern page - Desktop - Setup Baseline",
    { tag: "@setupProject" },
    async ({ page }) => {
      await allure.severity("minor");
      const currentScreenshot = `${currentDir}/desktop/Modern-page-current.png`;
      const baselineScreenshot = `${baselineDir}/desktop/Modern-page-baseline.png`;
      ...
      await uploadImage(`desktop/Modern-page-baseline.png`, baselineScreenshot);
      console.log("Baseline created. Run the test again for comparisons.");
    }
  );

  test(
    "Modern page - Desktop - Validate Mismatch",
    { tag: "@validation" },
    async ({ page }) => {
      await allure.severity("minor");
      const currentScreenshot = `${currentDir}/desktop/Modern-page-current.png`;
      const baselineScreenshot = `${baselineDir}/desktop/Modern-page-baseline.png`;
      const diffScreenshot = `${diffDir}/desktop/Modern-page-diff.png`;
  ...
  test(
    ...
    async ({ page }) => {
      ...
      console.log(`Creating baseline for ${test.info().title} test...`);
      fs.copyFileSync(currentScreenshot, baselineScreenshot);
      await uploadImage(`mobile/Modern-page-baseline.png`, baselineScreenshot);
      console.log("Baseline created. Run the test again for comparisons.");
    }
  );
